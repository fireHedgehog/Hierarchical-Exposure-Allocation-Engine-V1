...
The manual pipeline's six compute stages — from fetching source data through classifying the market regime, ranking candidates, sizing exposure, and proposing an instrument — are real, running on free-tier data (FRED for macro, Yahoo for prices) over a database-seeded staging universe of 21 tradeable symbols. One stage (publishing the final snapshot) remains a placeholder. Some strategies are now built from smaller, independently swappable pieces rather than one fixed formula, and a research scorecard checks how much genuinely new information a candidate factor adds before it's trusted. See [engine milestones](docs/engine-milestones.md) for verified status and examples. Live paid-provider ingestion, a governed production universe, scheduling, broker connectivity, and order handling remain outside this milestone. The application is therefore a research and simulation environment; its records are not trading instructions.
