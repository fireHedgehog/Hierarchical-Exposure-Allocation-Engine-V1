...
2. **Risk budget** converts that context into an allowable level and direction of exposure.
3. **Cross-sectional allocation** compares opportunity sets and assigns capital among strategy and factor families.
4. **Symbol expression** translates an allocation into a stock, short, option, or defined-risk structure appropriate to the strength of the view.
5. **Portfolio constraints** reconcile net, gross, concentration, liquidity, factor, premium, and Greek exposures across all positions.
6. **Monitoring** measures realized behavior, decay, and regime fit so strategies can be promoted, reweighted, or retired.
...
- an Operations workspace for provider readiness, credential smoke tests, data inventory, manual pipeline runs, and strategy lifecycle records;
...
Further stages will follow the same inspectable pattern: regime state, risk budget, cross-sectional allocation, symbol expression, and portfolio constraints. Scheduling becomes appropriate only after repeated manual runs are reproducible and operationally safe.
