...
def test_raises_instead_of_fabricating_when_no_inputs() -> None:
    with pytest.raises(InsufficientAllocationDataError):
        compute_risk_envelope(CURRENT_VINTAGE_ADVERSE_FREQUENCY_MIXED, [])


def test_calm_confidence_gives_the_max_multiplier() -> None:
    envelope = compute_risk_envelope(CURRENT_VINTAGE_ADVERSE_FREQUENCY_SUPPORTIVE, _inputs())
    assert envelope.gross_multiplier == pytest.approx(1.5)


def test_stressed_confidence_gives_the_min_multiplier() -> None:
    envelope = compute_risk_envelope(CURRENT_VINTAGE_ADVERSE_FREQUENCY_ADVERSE, _inputs())
    assert envelope.gross_multiplier == pytest.approx(0.5)


def test_middle_confidence_sits_strictly_between() -> None:
    envelope = compute_risk_envelope(CURRENT_VINTAGE_ADVERSE_FREQUENCY_MIXED, _inputs())
...
def test_this_is_the_real_bug_naive_v1_had_at_the_actual_production_range() -> None:
    ...
    stressed = compute_risk_envelope(CURRENT_VINTAGE_ADVERSE_FREQUENCY_ADVERSE, _inputs())
    middle = compute_risk_envelope(CURRENT_VINTAGE_ADVERSE_FREQUENCY_MIXED, _inputs())
    calm = compute_risk_envelope(CURRENT_VINTAGE_ADVERSE_FREQUENCY_SUPPORTIVE, _inputs())
