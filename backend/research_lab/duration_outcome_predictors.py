"""Scratch script for docs/hypotheses/macro-research/duration-outcome-predictors.md.

Layer 1 vs. layer 3's Duration dimension: does each free input signal
predict DGS10 (10Y yield)'s own forward change? Falling yield = bond bull,
rising = bond bear. Same continuous-target method as H-MACRO02/04/05.
Read-only against the sealed dataset.

Run: .venv/bin/python -m backend.research_lab.duration_outcome_predictors
"""
...
def main() -> None:
    ...
    dgs10 = _series(connection, dataset_id, "DGS10")
    print(f"Dataset: {dataset_id}")
    print(f"DGS10 (target): {len(dgs10)} real observations, {dgs10[0][0]} to {dgs10[-1][0]}")
    print()

    sample_points = dgs10[::STRIDE_DAYS]
    ...
    for series_id in CANDIDATE_SERIES:
        ...
        for observation_date, target_value in sample_points:
            ...
            future_value = _nearest_value_on_or_after(dgs10, future_target_date)
