"""Scratch script for docs/hypotheses/macro-research/composite-forward-risk-oos.md.

Out-of-sample split of composite_forward_risk.py's finding, requested
directly: a real, chronological split at 2019-01-01 (not random -- shuffling
a time series would leak future information through the trailing-window
z-scores), run with the exact same naive methodology on both halves, no
refitting -- nothing in this composite was ever fitted to begin with, so
this is a real replication check, not overfitting protection in the
classic sense. In-sample: 2004-2018 (includes 2008). Out-of-sample:
2019-2026 (includes the 2020 COVID crash and the 2022 hiking cycle, so the
holdout isn't a quiet stretch where a null result would be uninformative
either way). Read-only against the sealed dataset.

Run: .venv/bin/python -m backend.research_lab.composite_forward_risk_oos
"""
...
def main() -> None:
    ...
    SPLIT_DATE = "2019-01-01"  # disclosed, chronological, not chosen after seeing either half's result

    for label, date_filter in (
        ("IN-SAMPLE (2004 - 2018)", lambda d: d < SPLIT_DATE),
        ("OUT-OF-SAMPLE (2019 - 2026)", lambda d: d >= SPLIT_DATE),
    ):
        print(f"\n########## {label} ##########")
        for forward_days in FORWARD_WINDOWS:
            ...
            if n < 15:
                print("Too few samples in this window -- skipping stats.\n")
                continue
