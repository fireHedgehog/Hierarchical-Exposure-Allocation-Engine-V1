"""Scratch script for docs/hypotheses/macro-research/credit-outcome-predictors.md.

Layer 1 vs. layer 3's Credit dimension: does each free input signal predict
BAMLH0A0HYM2 (HY OAS spread)'s own forward 13-week change? Same continuous-
target methodology as H-MACRO02 (balance sheet), not H-MACRO01's discrete
events -- spreads are a continuous price, not a discrete policy action.
Real constraint disclosed up front: the target series itself only has real
point-in-time history from 2023-08-28 (see gap-down-fill.md's sibling
finding), so this whole test is bounded to that ~3-year window regardless
of how deep any individual predictor's own history goes. Read-only against
the sealed dataset.

Run: .venv/bin/python -m backend.research_lab.credit_outcome_predictors
"""
...
MIN_SAMPLES = 15  # disclosed lower floor -- the target's own real history is short by construction
FORWARD_DAYS = 91
STRIDE_WEEKS_IN_DAYS = 10  # ~2 real observations/month sampled, not every daily print

CANDIDATE_SERIES = [
    # DFEDTAR excluded: permanently discontinued 2008-12-15, so within this
    # target's own 2023+ real window it is a frozen constant -- not a valid
    # predictor for this specific test, not a bug.
    "INDPRO", "CPIAUCSL", "PPIACO", "PCEPILFE", "PAYEMS", "NFCI", "VIXCLS",
    "DGS10", "DGS30", "GDPC1", "MTSDS133FMS", "ICSA",
    "T10YIE", "T5YIE", "DFII10", "DFII30", "BAMLC0A0CM",
    "SOFR", "IORB", "DFEDTARU", "DFEDTARL", "WTREGEN", "WALCL",
]
...
def main() -> None:
    ...
    target = _series(connection, dataset_id, "BAMLH0A0HYM2")
    print(f"Dataset: {dataset_id}")
    print(f"BAMLH0A0HYM2 (target): {len(target)} real observations, {target[0][0]} to {target[-1][0]}")
    print()

    sample_points = target[::STRIDE_WEEKS_IN_DAYS]
    ...
    for series_id in CANDIDATE_SERIES:
        ...
        for observation_date, target_value in sample_points:
            ...
            future_value = _nearest_value_on_or_after(target, future_target_date)
            if future_value is None:
                continue
            indicator_value = _nearest_prior_value(indicator, observation_date)
            if indicator_value is None:
                continue
            forward_change = (future_value - target_value) / abs(target_value)
