...
CREATE TABLE IF NOT EXISTS research_metric_catalog (
    ...
    -- Independent from `category` (what KIND of metric) -- this is WHAT LEVEL
    -- of the strategy hierarchy it evaluates, matching the granularity that
    -- already exists structurally in strategies/strategy_versions/
    -- strategy_components:
    --   component  one named sub-signal alone (a strategy_components row) --
    --              IC, decay, turnover of one factor by itself.
    --   ensemble   relationships AMONG a strategy's own components, not yet
    --              tradable alone -- correlation, effective number of bets,
    --              marginal contribution, multiple-hypothesis correction.
    --   strategy   one strategy's own realized, combined backtest output --
    --              CAGR, Sharpe, drawdown, PBO -- the tier an optimizer
    --              (e.g. a genetic algorithm) could actually fit against.
    --   desk       cross-strategy, whole-portfolio -- exposure limits,
    --              neutralization, concentration, universe governance.
    -- A cross-cutting category (data integrity, robustness, trading reality)
    -- does not get its own level -- each of its metrics is tagged by what it
    -- actually checks, so this stays one clean axis rather than two
    -- overlapping ones.
    granularity TEXT NOT NULL CHECK (granularity IN ('component', 'ensemble', 'strategy', 'desk')),
    label TEXT NOT NULL,
    unit TEXT,
    description TEXT NOT NULL,
...
INSERT OR IGNORE INTO research_metric_catalog (metric_key, category, granularity, label, unit, description, applicable_families_json, sort_order) VALUES
    -- 1. Data integrity
    ('point_in_time_data', 'data_integrity', 'strategy', 'Point-in-time data', NULL, 'Every input value was actually available (observed_at/available_at) at or before the decision timestamp used to test it.', '[]', 1),
    ('no_look_ahead_bias', 'data_integrity', 'strategy', 'No look-ahead bias', NULL, 'No feature or label uses information not yet knowable at the simulated decision time.', '[]', 2),
    ('no_survivorship_bias', 'data_integrity', 'desk', 'No survivorship bias', NULL, 'The evaluated universe includes symbols that were later delisted or removed, not only current survivors.', '[]', 3),
    ('corporate_action_adjustment', 'data_integrity', 'strategy', 'Corporate action adjustment', NULL, 'Splits, dividends, and other corporate actions are correctly reflected in the price series used.', '[]', 4),
    ('delisting_returns', 'data_integrity', 'strategy', 'Delisting returns', NULL, 'A delisted security'' final return is captured, not silently dropped from the sample.', '[]', 5),
    ('universe_membership_at_t', 'data_integrity', 'desk', 'Universe membership at time t', NULL, 'Eligibility is evaluated against the universe as it stood at each historical decision date, not today''s universe applied retroactively.', '[]', 6),
    ('publication_reporting_lag', 'data_integrity', 'component', 'Publication / reporting lag', NULL, 'The real gap between a value''s observation date and its actual public availability date is modeled, not assumed to be zero.', '[]', 7),
    -- 2. Signal / factor validation
    ('factor_ic', 'signal_validation', 'component', 'Factor IC (Information Coefficient)', 'correlation', 'Pearson correlation between a factor''s value and the real forward return it is meant to predict.', '[]', 10),
    ('factor_rank_ic', 'signal_validation', 'component', 'Rank IC', 'correlation', 'Spearman rank correlation between a factor''s value and the real forward return -- robust to outliers and nonlinearity that Pearson IC is not.', '[]', 11),
    ('ic_mean', 'signal_validation', 'component', 'IC mean', 'correlation', 'Mean IC across multiple evaluation periods.', '[]', 12),
    ('ic_std', 'signal_validation', 'component', 'IC std', 'correlation', 'Standard deviation of IC across multiple evaluation periods -- how stable the relationship is, not just its average.', '[]', 13),
    ('icir', 'signal_validation', 'component', 'ICIR', 'ratio', 'IC mean divided by IC std -- an information-ratio-style measure of a factor''s risk-adjusted predictive consistency.', '[]', 14),
    ('factor_return', 'signal_validation', 'component', 'Factor return', 'fraction', 'Return of a portfolio built purely from this factor''s ranking.', '[]', 15),
    ('long_short_spread', 'signal_validation', 'component', 'Long-short spread', 'fraction', 'Return difference between the factor''s top and bottom ranked buckets.', '[]', 16),
    ('factor_hit_rate', 'signal_validation', 'component', 'Hit rate', 'fraction', 'Share of periods where the factor''s ranking direction matched the realized outcome direction.', '[]', 17),
    ('factor_turnover', 'signal_validation', 'component', 'Factor turnover', 'fraction', 'How much the factor''s cross-sectional ranking churns period to period -- high turnover raises real trading cost for the same signal.', '[]', 18),
    ('decay_half_life', 'signal_validation', 'component', 'Decay / half-life', 'periods', 'How many periods it takes for the factor''s predictive power (IC) to fall to half its initial value.', '[]', 19),
    ('factor_correlation', 'signal_validation', 'ensemble', 'Factor correlation', 'correlation', 'Pairwise correlation between this factor and every other registered factor in its family -- the raw input to effective-number-of-bets and redundancy detection.', '[]', 20),
    ('effective_number_of_bets', 'signal_validation', 'ensemble', 'Effective number of bets', 'count', 'PCA-based measure of how many genuinely independent bets a factor family actually represents -- N correlated factors are worth far fewer than N independent ones.', '[]', 21),
    ('exposure_correlation', 'signal_validation', 'ensemble', 'Exposure correlation', 'correlation', 'Correlation between this factor''s resulting portfolio exposure and another factor''s or the market''s.', '[]', 22),
    ('marginal_contribution', 'signal_validation', 'ensemble', 'Marginal contribution', 'fraction', 'The factor''s unique contribution to combined predictive power after accounting for its correlation with already-registered factors.', '[]', 23),
    -- 4. Backtest performance metrics
    ('cagr', 'backtest_performance', 'strategy', 'CAGR / annualized return', 'fraction', 'Compound annual growth rate of the strategy''s equity curve.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 30),
    ('annualized_volatility', 'backtest_performance', 'strategy', 'Annualized volatility', 'fraction', 'Standard deviation of returns, annualized.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 31),
    ('sharpe_ratio', 'backtest_performance', 'strategy', 'Sharpe ratio', 'ratio', 'Annualized mean return divided by annualized volatility.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 32),
    ('sortino_ratio', 'backtest_performance', 'strategy', 'Sortino ratio', 'ratio', 'Like Sharpe, but only penalizing downside volatility.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 33),
    ('max_drawdown', 'backtest_performance', 'strategy', 'Maximum drawdown', 'fraction', 'Largest peak-to-trough decline in the equity curve.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 34),
    ('drawdown_duration', 'backtest_performance', 'strategy', 'Drawdown duration', 'periods', 'How long the strategy stayed below its prior peak.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 35),
    ('calmar_ratio', 'backtest_performance', 'strategy', 'Calmar ratio', 'ratio', 'CAGR divided by maximum drawdown.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 36),
    ('trade_hit_rate', 'backtest_performance', 'strategy', 'Hit rate / win rate', 'fraction', 'Share of closed trades with a positive return.', '["single_name_timing"]', 37),
    ('payoff_ratio', 'backtest_performance', 'strategy', 'Payoff ratio', 'ratio', 'Average winning trade divided by average losing trade.', '["single_name_timing"]', 38),
    ('profit_factor', 'backtest_performance', 'strategy', 'Profit factor', 'ratio', 'Gross profit divided by gross loss.', '["single_name_timing"]', 39),
    ('value_at_risk', 'backtest_performance', 'strategy', 'Value at Risk (VaR)', 'fraction', 'Loss threshold not expected to be exceeded at a given confidence level.', '["single_name_timing","cross_sectional_discovery","instrument_expression","portfolio_construction"]', 40),
    ('expected_shortfall', 'backtest_performance', 'strategy', 'Expected Shortfall (CVaR)', 'fraction', 'Average loss in the tail beyond the VaR threshold.', '["single_name_timing","cross_sectional_discovery","instrument_expression","portfolio_construction"]', 41),
    ('return_skewness', 'backtest_performance', 'strategy', 'Skewness', 'ratio', 'Asymmetry of the return distribution.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 42),
    ('return_kurtosis', 'backtest_performance', 'strategy', 'Kurtosis', 'ratio', 'Tail-heaviness of the return distribution relative to normal.', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 43),
    ('portfolio_turnover', 'backtest_performance', 'strategy', 'Turnover', 'fraction', 'How much the resulting positions actually trade, period to period -- distinct from factor_turnover (ranking churn) upstream of it.', '["single_name_timing","cross_sectional_discovery","portfolio_construction"]', 44),
    ('market_beta', 'backtest_performance', 'strategy', 'Market beta', 'ratio', 'Sensitivity of strategy returns to the broad market.', '["single_name_timing","cross_sectional_discovery","instrument_expression","portfolio_construction"]', 45),
    ('jensens_alpha', 'backtest_performance', 'strategy', 'Jensen''s alpha', 'fraction', 'Return in excess of what market beta alone would predict (CAPM residual).', '["single_name_timing","cross_sectional_discovery","instrument_expression","portfolio_construction"]', 46),
    -- 5. Robustness / statistical validation
    ('in_sample_out_of_sample', 'robustness_validation', 'strategy', 'In-sample / out-of-sample', NULL, 'Performance is compared between the period a rule was developed on and a genuinely unseen later period.', '[]', 50),
    ('walk_forward_validation', 'robustness_validation', 'strategy', 'Walk-forward validation', NULL, 'Repeated re-fit-then-test on rolling, non-overlapping forward windows.', '[]', 51),
    ('rolling_window_analysis', 'robustness_validation', 'strategy', 'Rolling window analysis', NULL, 'Metric stability measured across overlapping rolling windows rather than one fixed period.', '[]', 52),
    ('parameter_sensitivity', 'robustness_validation', 'strategy', 'Parameter sensitivity', NULL, 'How much results change under small, reasonable changes to hand-picked parameters -- a cliff-edge result is a red flag.', '[]', 53),
    ('subsample_stability', 'robustness_validation', 'strategy', 'Subsample stability', NULL, 'Results hold up across different sub-periods or sub-universes, not just the full sample.', '[]', 54),
    ('regime_analysis', 'robustness_validation', 'strategy', 'Regime analysis', NULL, 'Performance broken out by macro/vol regime rather than reported as one blended average.', '[]', 55),
    ('bootstrap_monte_carlo', 'robustness_validation', 'strategy', 'Bootstrap / Monte Carlo', NULL, 'Resampling-based confidence bounds on a metric, rather than a single point estimate.', '[]', 56),
    ('multiple_hypothesis_testing', 'robustness_validation', 'ensemble', 'Multiple-hypothesis testing', NULL, 'How many hypotheses were actually tried before this one was reported -- the honest denominator behind any p-value.', '[]', 57),
    ('false_discovery_rate', 'robustness_validation', 'ensemble', 'False discovery rate', 'fraction', 'Benjamini-Hochberg-style correction already implemented and used in this project (engine/research/significance.py) for macro-factor and momentum-horizon testing.', '[]', 58),
    ('deflated_sharpe_ratio', 'robustness_validation', 'strategy', 'Deflated Sharpe ratio', 'ratio', 'Sharpe ratio adjusted for the number of trials, track record length, and return non-normality (Bailey & Lopez de Prado, 2014).', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 59),
    ('probability_of_backtest_overfitting', 'robustness_validation', 'strategy', 'Probability of Backtest Overfitting (PBO)', 'fraction', 'Combinatorially-symmetric cross-validation estimate of the probability that the selected strategy is overfit (Bailey, Borwein, Lopez de Prado & Zhu, 2017).', '["single_name_timing","cross_sectional_discovery","instrument_expression"]', 60),
    -- 6. Trading reality
    ('transaction_costs', 'trading_reality', 'strategy', 'Transaction costs', 'fraction', 'Commissions and fees actually deducted from simulated returns.', '["single_name_timing","cross_sectional_discovery","instrument_expression","portfolio_construction"]', 70),
    ('bid_ask_spread', 'trading_reality', 'strategy', 'Bid-ask spread', 'fraction', 'Real quoted spread cost paid crossing the market on entry and exit, not a mid-price fill.', '["single_name_timing","instrument_expression"]', 71),
    ('slippage', 'trading_reality', 'strategy', 'Slippage', 'fraction', 'Difference between the intended and actually achievable execution price.', '["single_name_timing","instrument_expression"]', 72),
    ('market_impact', 'trading_reality', 'strategy', 'Market impact', 'fraction', 'Price movement caused by the order itself, scaling with size relative to liquidity.', '["single_name_timing","instrument_expression","portfolio_construction"]', 73),
    ('borrow_cost', 'trading_reality', 'strategy', 'Borrow cost', 'fraction', 'Cost of borrowing shares to hold a short position.', '["single_name_timing","instrument_expression"]', 74),
    ('short_availability', 'trading_reality', 'strategy', 'Short availability', NULL, 'Whether shares are actually borrowable for a proposed short, not merely assumed available.', '["single_name_timing","instrument_expression"]', 75),
    ('liquidity_constraints', 'trading_reality', 'strategy', 'Liquidity constraints', NULL, 'Position size checked against real traded volume, not sized as if infinitely liquid.', '["single_name_timing","instrument_expression","portfolio_construction"]', 76),
    ('adv_participation_limit', 'trading_reality', 'strategy', 'ADV participation limit', 'fraction', 'Order size capped as a fraction of average daily volume.', '["single_name_timing","instrument_expression","portfolio_construction"]', 77),
    ('execution_delay', 'trading_reality', 'strategy', 'Execution delay', 'periods', 'Real gap between signal generation and achievable fill, not same-bar fills assumed for free.', '["single_name_timing","instrument_expression"]', 78),
    ('capacity', 'trading_reality', 'strategy', 'Capacity', 'usd', 'Maximum capital the strategy can absorb before its own trading materially degrades its edge.', '["single_name_timing","cross_sectional_discovery","instrument_expression","portfolio_construction"]', 79),
    -- 7. Portfolio / risk layer
    ('position_sizing', 'portfolio_risk', 'desk', 'Position sizing', NULL, 'The rule mapping conviction/signal strength to an actual position size.', '["instrument_expression","portfolio_construction"]', 90),
    ('volatility_targeting', 'portfolio_risk', 'desk', 'Volatility targeting', NULL, 'Position size scaled to hit a target portfolio volatility rather than a fixed notional.', '["portfolio_construction"]', 91),
    ('risk_budgeting', 'portfolio_risk', 'desk', 'Risk budgeting', NULL, 'Risk, not capital, allocated across sleeves/positions as the primary budget unit.', '["portfolio_construction"]', 92),
    ('factor_exposure_limits', 'portfolio_risk', 'desk', 'Factor exposure limits', NULL, 'Portfolio-level caps on net exposure to any one factor.', '["portfolio_construction"]', 93),
    ('sector_industry_constraints', 'portfolio_risk', 'desk', 'Sector / industry constraints', NULL, 'Caps on concentration within any one sector or industry.', '["portfolio_construction"]', 94),
    ('single_name_concentration_limits', 'portfolio_risk', 'desk', 'Single-name concentration limits', NULL, 'Caps on how much of the portfolio any one security can represent.', '["portfolio_construction"]', 95),
    ('beta_neutralization', 'portfolio_risk', 'desk', 'Beta neutralization', NULL, 'Whether and how market-beta exposure is hedged out of the portfolio.', '["portfolio_construction"]', 96),
    ('dollar_neutralization', 'portfolio_risk', 'desk', 'Dollar neutralization', NULL, 'Long and short dollar exposure balanced to a target net.', '["portfolio_construction"]', 97),
    ('factor_neutralization', 'portfolio_risk', 'desk', 'Factor neutralization', NULL, 'Unwanted factor exposures hedged out so a position expresses the intended bet only.', '["portfolio_construction"]', 98),
    ('correlation_covariance_control', 'portfolio_risk', 'desk', 'Correlation / covariance control', NULL, 'Portfolio construction accounts for the real covariance between positions, not just their individual risk.', '["portfolio_construction"]', 99),
    ('gross_net_exposure_limits', 'portfolio_risk', 'desk', 'Gross / net exposure limits', NULL, 'Hard caps on total long+short (gross) and long-short (net) exposure.', '["portfolio_construction"]', 100),
    ('drawdown_control', 'portfolio_risk', 'desk', 'Drawdown control', NULL, 'A rule that reduces risk in response to realized drawdown, rather than a static allocation regardless of recent losses.', '["portfolio_construction"]', 101);
