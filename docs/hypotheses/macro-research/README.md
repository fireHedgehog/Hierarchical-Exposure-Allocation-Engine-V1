...
| ~~Compare cluster-equal-weighting against IC-weighted alternatives~~ | **Answered, and settled.** [H-MACRO11](factor-breadth-weighting.md): after fixing a real sign bug and testing against the composite's real target (drawdown probability, not return), the existing cluster-equal design beats IC-weighting out-of-sample. Real evidence for keeping naive-v3's design, not a reason to change it |
