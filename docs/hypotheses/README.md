...
| [Asset selection research](asset-selection-research/README.md) (subfolder) | 13-asset real sleeve universe (GLD-anchored), own index | concluded (this arc) | 4 papers: H-SECT01 (leadership persistence, rejected) → H-SECT02 (regime-conditioned relative return, confirmed, OOS-checked) → H-SECT03 (driver decomposition, rejected — no single driver) → H-SECT04 (regime-tilted allocation backtest, rejected — real correlation doesn't survive becoming an allocation rule, +0.008 OOS Sharpe). Net: `macro_regime_composite` stays scoped to gross exposure only. | n/a |
