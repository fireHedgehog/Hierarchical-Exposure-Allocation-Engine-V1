...
| --- | --- | --- | --- | --- |
| [Macro research](macro-research/README.md) (subfolder) | 3-layer input/response/outcome framework, own index | in progress | 11 real papers (H-MACRO01-11 + Warsh). Composite (`macro_regime_composite`) live at naive-v3 with real out-of-sample-validated forward-drawdown evidence (H-MACRO09). H-MACRO10 found and fixed a real directional bug in the exposure formula (naive-v2, live). H-MACRO11: a real, walk-forward IC-weighted combination of the 13 factors nearly doubles OOS IC vs. naive-v3's cluster-equal weighting — real, substantial, but flagged (wrong target tested, real next step before production). | n/a |
