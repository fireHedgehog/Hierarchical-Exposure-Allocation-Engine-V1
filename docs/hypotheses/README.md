...
| [Asset selection research](asset-selection-research/README.md) (subfolder) | 13-asset real sleeve universe (GLD-anchored), own index | concluded (this arc) | 5 papers: H-SECT01 (leadership persistence, rejected) → H-SECT02 (regime-conditioned relative return, confirmed) → H-SECT03 (driver decomposition, rejected) → H-SECT04 (allocation backtest, rejected, +0.008 OOS Sharpe) → H-SECT05 (beta-adjustment narrows H-SECT02 to 3/24; only `XLU` survives every check). Net: `macro_regime_composite` stays scoped to gross exposure only. | n/a |
