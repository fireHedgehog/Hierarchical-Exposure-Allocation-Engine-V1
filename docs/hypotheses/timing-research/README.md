...
| Paper | Status | Covers |
| --- | --- | --- |
| [VIX-percentile VXX entry timing](vix-percentile-vxx-entry.md) (H-TIME01) | v1 confirmed (partial); v2 inconclusive | New real data this paper required: `VXX` added and fetched (real history only reaches 2018, not 2009). v1: days since `VIXCLS` was last elevated real-predicts `VXX`'s forward return (r=+0.26 to +0.36, significant); percentile level does not. v2 (user's reframe — event study, not continuous IC): does a real compression episode (VIX <15 for 21 days) precede a real explosion — direction consistent at all 4 tests, closest at p=0.063 (full 2004-2026 VIX-native sample, n=27), not yet significant. Real, not yet decisive. |
