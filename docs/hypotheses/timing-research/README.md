...
| Paper | Status | Covers |
| --- | --- | --- |
| [VIX-percentile VXX entry timing](vix-percentile-vxx-entry.md) (H-TIME01) | v1 confirmed (partial); v2/v3 real, thin-capacity signal, not a rejection | New real data this paper required: `VXX` added and fetched (real history only reaches 2018, not 2009). v1: days since `VIXCLS` was last elevated real-predicts `VXX`'s forward return (significant, both windows); percentile level does not. v2/v3 (event study, then a disciplined threshold scan): compression → explosion, direction consistent across every design, never conventionally significant — reframed per direct correction: ~1.2 qualifying episodes/year means a real effect would *always* look marginal at this sample size. Watch-and-size-small, not a systematic rule. |
| [Broad-index exposure policy](broad-index-exposure-policy.md) (H-TIME02) | Concluded-confirmed | A deliberate reframe: every trend test in this project (including #1's rejections and `asset-selection-research/`'s trend-conditioned addenda) asked "does state predict forward return" (IC) and found nothing. This paper asked the different, real question a slow trend/vol exposure filter is actually for — does the *full path* (drawdown, time underwater) improve enough to be worth the CAGR given up. Confirmed: real, consistent Sharpe and max-drawdown improvement over static buy-and-hold on SPY and QQQ, stable across 4 MA lengths (150/180/200/220), holding in-sample and OOS. Directly ruled out the "just less invested on average" artifact via a same-mean-exposure static-multiplier control. Not yet promoted — composition with the live macro-based exposure scaling (H-MACRO10) is separate, undone work. |
