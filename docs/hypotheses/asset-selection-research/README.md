...
| Question | Status |
| --- | --- |
| Section leadership persistence (#7) | **Rejected, twice.** [H-SECT01](section-leadership-persistence.md): quarterly persistence = exactly chance (33.3%, p=0.52); still exactly chance split by real SPY trend regime (bull/bear/mixed) too. |
| Real market-trend-regime filter (SPY price vs. its own moving average — 5d/20d combo and 50MA both tried) — a genuine, pre-specified, explainable conditioning variable, distinct from `macro_regime_composite` | Applied twice now (H-SECT01's addendum, H-SECT02's addendum), both clean rejections. Real, informative pattern: macro-regime conditioning keeps finding signal, price-trend conditioning hasn't yet at any window tried. Still a real candidate for the reversal edge's own trend-dependence (never tried) or H-SECT10 — but the emerging pattern says lower expected value than another macro-conditioned test |
...
| Paper | Status | Covers |
| --- | --- | --- |
| [Section leadership persistence](section-leadership-persistence.md) (H-SECT01) | concluded-rejected | Real quarterly-horizon test: P(leader next quarter \| leader this quarter) = 33.3%, exactly chance (p=0.52). A same-session daily-overlapping-window test had appeared to confirm persistence (p=0.001) — turned out to be a mechanical rolling-window artifact, not a real effect; caught and superseded before promotion. v2 addendum: re-tested split by a real SPY trend regime (bull/bear/mixed, 5d/20d moving averages) — still exactly chance in every regime (36.4%/33.3%/30.8%), rules out "the pooled null hid a real bull-market effect." |
| [Regime-conditioned sleeve relative return](regime-conditioned-sleeve-return.md) (H-SECT02) | concluded-confirmed, narrowed by H-SECT05; v2 (trend-conditioned) rejected | Different mechanism than H-SECT01: macro state, not trend, predicting relative return. 11 of 24 sleeve/window tests significant after correction (~1.2 expected by chance) — but see H-SECT05, most of this doesn't survive beta-adjustment. v2 addendum: same measure conditioned on a real SPY-50MA trend filter instead of macro regime, extended to include `SMH`/`IGV` — clean rejection, 0/28. Macro-regime conditioning keeps finding real signal; price-trend conditioning (tried twice now) hasn't. |
...
| [Regime-tilted allocation backtest](regime-tilted-allocation-backtest.md) (H-SECT04) | concluded-rejected (v1 + v2 + v3) | v1: binary tilt on 4 sleeves, OOS Sharpe +0.008, rejected. v2 (Fundamental Law test — all 12 sleeves, IC-weighted, real effective breadth): real effective breadth is only ~2.4 (not 12) — a standalone-valuable finding. Grinold predicts IR≈+0.22; realized is +0.005-0.014 (real, positive, but 15-40x smaller). v3 (extended to 14 sleeves incl. `SMH`/`IGV`): same result, breadth barely moves (2.44→2.48) — the bottleneck is real correlation, not ticker count. Compare against H-MACRO11: the same breadth framework found a *much* bigger real improvement at the macro-factor level — this layer's problem is thin real signal, not the method. |
