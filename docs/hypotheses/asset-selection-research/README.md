...
| Real market-trend-regime filter (SPY price vs. its own 5d/20d moving average) — a genuine, pre-specified, explainable conditioning variable, distinct from `macro_regime_composite` | New methodology, available for reuse. Applied once (H-SECT01's addendum, null). Real candidate for re-testing H-SECT10 (theme relative strength, also rejected pooled) or the reversal edge's own trend-dependence — not yet done |
