...
| [Regime-tilted allocation backtest](regime-tilted-allocation-backtest.md) (H-SECT04) | concluded-rejected (v1 + v2) | v1: binary tilt on 4 sleeves, OOS Sharpe +0.008, rejected. v2 (Fundamental Law test — all 12 sleeves, IC-weighted, real effective breadth): real effective breadth is only ~2.4 (not 12) — a standalone-valuable finding. Grinold predicts IR≈+0.22; realized is +0.005-0.014 (real, positive, but 15-40x smaller), due to in-sample IC inflation, the long-only constraint, and uncosted turnover. Same conclusion, reached properly this time. |
