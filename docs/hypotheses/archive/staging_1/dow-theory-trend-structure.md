...
| 2026-08-26 | Rerun on the real 2004-2026 dataset (post-0.38 extension, now including 2008), same script, same swing detector | **Rejected, replicated.** r=-0.030 (adjusted p<0.0001, n=21,480: 8,026 intact, 13,454 broken) -- was r=-0.039 on 2016-26. Mean forward return: intact 0.97%, broken 1.37% -- same direction, similar magnitude. | The rejection holds up on the longer window including 2008, not just the bull-decade sample -- a real, replicated result, not weakened the way time-series-momentum's rejection was on the same rerun. |
| 2026-08-26 | Follow-up: real orthogonality check across all five price/vol signals this session, `research_lab/orthogonality_check.py`, same dataset — the "four independent tests" claim above was asserted, not verified; checked, not left standing | **Partly wrong, corrected.** `low_vol_63d` and `max_return_21d` are r=+0.78 — not independent, nearly the same bet (a big single-day spike mechanically inflates the same window's realized vol). But `dow_structure_intact` correlates weakly with everything else (\|r\| 0.02–0.12), and `ts_momentum_12m` only modestly (0.12–0.18) — effective number of bets across all 5 signals = 3.84 of 5. | The honest revision: not "four independent tests," and not "one test four times" either — roughly four real, mostly-independent signals with one genuinely redundant pair, still landing the same direction. Weaker than originally claimed, still real. Recorded here because this paper is where the strongest version of the original claim was made. |
