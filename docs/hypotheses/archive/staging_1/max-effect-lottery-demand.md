...
| 2026-08-26 | Follow-up orthogonality check (`research_lab/orthogonality_check.py`), triggered by a later, separate Dow Theory test — see `dow-theory-trend-structure.md`'s own follow-up entry for the full result | This signal and `low-volatility-anomaly.md`'s are r=+0.78 with each other — genuinely the same bet, not independent (a big single-day spike mechanically inflates the same window's realized vol). The two rejections agreeing was real but expected, not two separate pieces of evidence. | Correction to this paper's own earlier framing ("three independent specifications"): this one and low-vol's specifically were never independent of each other, even though the direction agreeing with time-series-momentum's separate, largely uncorrelated signal is still real. |
