...
1. Manually audit [H-TIME-S2-002](hypotheses/staging_v2/timing/h-time-s2-002-confirmation-and-path-surface.md). If accepted, implement one disposable event-tensor loop and run all four frozen families; do not split it into per-indicator scripts, fetch new provider data, open H-XSEC-S2-003's locked folds, or alter the Timing product from an S2 result.
