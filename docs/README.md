...
- Research Staging V2 is active under [`docs/hypotheses/staging_v2/`](hypotheses/staging_v2/README.md): four concise type READMEs carry each loop and result table. Macro S2/S6 now have an exact-runtime, block-permutation audit; S3-CV's current-vintage numeric translation is accepted for staging (`55.4/100` environment position and `21.5%` six-month adverse-frequency reference for the current mixed state), while calibrated probability remains blocked by missing historical release-time PIT. H-MACRO-S4-002 now has one independent pre-run design for a long-only `0x-1.5x` risk-appetite policy; its literature choices, utility scaling, financing convention, and finite engineering grid remain unfrozen and have no production use. Prior experiments remain under [`archive/staging_1/`](hypotheses/archive/staging_1/README.md) only as temporary context.
...
1. Manually review and freeze H-MACRO-S4-002's literature-dependent utility scales, cash/financing convention, finite grid, and Phase A result contract before running it; current-vintage screening cannot promote a production policy.
