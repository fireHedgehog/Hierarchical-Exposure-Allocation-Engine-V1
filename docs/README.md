...
- Research Staging V2 is active under [`docs/hypotheses/staging_v2/`](hypotheses/staging_v2/README.md). Macro S2/S6 have an exact-runtime block-permutation audit; S2-002 routes all 26 stored FRED series through one 840-cell indicator-outcome matrix rather than forcing a single return or exposure winner. It found no stable SPY return-magnitude route but retained distinct damage, volatility, leadership, duration, inflation-pricing, policy-response, and gold relationships; VIX remained one useful fast input rather than the macro model. Timing H-TIME-S2-001 tested 36 matched dislocation-repair cells across SPY/QQQ/DIA/IWM: zero stable repair cells, 11 continuation-risk cells, 17 inconclusive, and eight rare-event insufficient. The immediate buy-the-break translation is rejected. Cross H-XSEC-S2-001 completed a 2,242-leader/4,474-control calendar-quarter loop, but H-XSEC-S6-001 later confirmed that its first-21-session, new-top-three rule systematically omits persistent leaders and confounds individual continuation with sector diffusion. H-XSEC-S2-002 then returned to a standard 15-factor monthly price/volume screen across 246 formations. Sector-neutral Amihud illiquidity is the one confirmed discovery relationship: full IC/spread `+.050/+1.71%` at 42 sessions and `+.061/+2.52%` at 63, remaining positive but weaker in holdout (`+.012/+1.07%`, `+.018/+1.81%`). H-XSEC-S5-001's next-session long-only rough check retained positive full/holdout excess through 50 bps assumed-cost stress with measured turnover, but real spread/impact is absent and historical capacity can be small; it remains an unregistered candidate. H-XSEC-S2-003 has now released Development only: its four continuous MA-strength curves were near zero/negative and none of E1/E5/EB20/ES passed the stock sector-excess, transition, drawdown, and curve-q gates, so Validation/Holdout remain locked. Its correlated four-index ES panel is only a timing follow-up observation. Simple low dollar volume did not retain holdout IC, while classic price momentum produced no useful relationship. H-THEME-S2-001 remains a diagnostic of the retired clock. Historical member work uses the current roster as a plainly labelled disposable discovery sample rather than blocking the screen. The dual-basis library gate is complete at 775/775 through 2026-08-27 ET. Optional SEC Item 2.02 rows remain a separate filing-time proxy, not an earnings timestamp. S3-CV's current-vintage numeric translation remains accepted for staging (`55.4/100` environment position and `21.5%` six-month adverse-frequency reference), while calibrated probability remains blocked by missing historical release-time PIT. H-MACRO-S4-002 remains inconclusive and authorizes no production translation. Prior experiments remain under [`archive/staging_1/`](hypotheses/archive/staging_1/README.md) only as temporary context.
...
1. Manually decide whether H-XSEC-S2-003's correlated four-index `ES` observation deserves a separately frozen Timing hypothesis. Do not open the Cross study's Validation/Holdout folds: Development nominated no stock candidate. Keep Amihud in the candidate ledger; neither result is registered in production. The 775/775 `stage-2` receipt gate through the fixed `2026-08-27` ET cutoff is complete.
