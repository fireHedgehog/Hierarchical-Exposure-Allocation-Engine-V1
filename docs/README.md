...
- Operations: a loopback-only, lower-frequency control plane provides an evidence-derived demo-to-real readiness map, a database-driven provider/capability onboarding roadmap, credential smoke tests, source freshness, dry/full manual run records, and strategy/research lifecycle views below the decision surfaces. The pipeline can also be run stage-by-stage (`stop_after`, e.g. "fetch data only") with a live, polled progress bar (stage/item counts); `fetch_data_stage` commits per-symbol and paces real Yahoo requests, so a killed process or a bad symbol never loses already-fetched data — see [engine milestones](engine-milestones.md) 0.91.
