...
- New "Known methodology limitations" section in `docs/hypotheses/README.md` (centralized, not scattered across every paper), with pointer notes added to `macro-research/README.md` and `asset-selection-research/README.md` scoping exactly which papers are affected. Gap #1's real remedy (per-date ALFRED vintage fetching) explicitly parked, per direct instruction -- a real, substantial software change, not undertaken to chase research validity at the cost of software stability.
- New `research_lab/macro_significance_robustness_check.py`: a real moving-block permutation test (shuffles contiguous blocks, not individual points, preserving real autocorrelation) applied to the two most consequential findings. H-MACRO09 holds under the more rigorous test (naive p=0.0002-0.0008 -> block-permutation p=0.0065-0.0075, still significant -- the naive p-values were real but overstated by 9-30x). H-MACRO11's correction gets *stronger*: cluster-equal survives (p=0.0185), the IC-weighted alternative -- which had looked marginally significant under naive testing (p=0.0097) -- does not (p=0.089) under the honest test. Both papers updated with the real re-check results.

### Research — H-MACRO11: factor breadth-weighting vs. cluster-equal — a real sign bug caught, and the corrected result reverses the exciting first pass

- Picked up an already-flagged, pre-scoped gap directly: `macro-research/README.md`'s restart-here table has carried "compare cluster-equal-weighting against IC-weighted alternatives" since the naive-v3 promotion. Applied the same Fundamental-Law-of-Active-Management framework just built for H-SECT04 v2 to the macro composite's own 13 factors (`research_lab/macro_factor_breadth_test.py`: real, walk-forward per-factor IC, real effective breadth via this project's own proven PCA machinery).
- Re-testing H-MACRO11 against the composite's actual, validated target (real drawdown probability, matching H-MACRO09 exactly, not forward return) surfaced an impossible result from the first pass: the IC-weighted composite showed a *positive* IC against drawdown risk -- its "stressed" tercile had *lower* real drawdown probability than its "calm" tercile. A real formula bug, not a real finding: `scoring_v3`'s per-factor contributions are already sign-oriented (negative = stress, consistently); the weighting formula used signed IC as the weight, which flips an already-correctly-signed, genuinely reliable factor's contribution, corrupting the composite. Fixed in both scripts: weight by `|IC|` (reliability) instead of signed IC.
- New `research_lab/macro_factor_breadth_test_drawdown_target.py` (the corrected re-test) plus the original script's fix. Corrected, real result against the target that actually matters: in-sample the IC-weighted alternative looks better (expected fitting bias), but out-of-sample the *existing* cluster-equal design wins -- real, significant (p=0.014) vs. the alternative's weaker, not-significant result (p=0.121). A real, disclosed, plausible mechanism: in-sample IC-weighting likely overfits to 2008's specific factor signature (`gdp`/`growth` dominated), which doesn't generalize to differently-shaped real crises (2020, 2022) -- the same "1/N puzzle" phenomenon documented in the finance literature (DeMiguel, Garlappi & Uppal 2009), where naive equal-weighting often beats optimized weighting out-of-sample precisely because optimization fits estimation-sample noise a naive scheme never tries to fit.
- H-MACRO11 recorded `concluded-rejected` against the real target -- a real reversal from the first pass, not a footnote. `scoring_v3.py` stays unchanged; this is now independent evidence *for* the existing naive-v3 design, not a case to change it.
