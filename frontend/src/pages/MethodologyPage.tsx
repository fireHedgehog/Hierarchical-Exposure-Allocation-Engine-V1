...
const LAYERS: Layer[] = [
  {
    ...
    summary: "8 real FRED series -> a single regime label and confidence, scored as each factor's surprise against its own trailing statistical expectation.",
    topParameter: "composite = sum(w_i * surprise_score_i) across 8 factors -- weights and expectation windows in the Registry record's parameters.",
    literature: [
  ...
  },
  {
    ...
    summary: "Real prices -> a blended 1M/3M/6M momentum score, ranked against peers. Horizon weights come from a real per-run significance test, not a fixed split.",
    topParameter: "blend = sum(w_h * r_h) for h in {1m,3m,6m}; w from pooled Pearson IC test, Benjamini-Hochberg corrected -- see Registry record.",
    literature: [
  ...
  },
  {
    ...
    topParameter: "entry: MACD bullish crossover. exit: MACD bearish crossover OR RSI(14) >= 70 -- each condition owned by its own component (macd_crossover, rsi_overbought_exit).",
    literature: [
      "Appel (2005). Technical analysis: Power tools for active investors. FT Press.",
